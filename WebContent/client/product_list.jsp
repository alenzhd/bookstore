...
<head>
	...
	<script>
		function changePageNum(){
			...
			window.location.href="http://localhost:8080/mybookStore/showProductByPage?currentCount="+currentCount+"&category="+category;
		}
	</script>
</head>
					...
					<table cellspacing="0" class="listcontent">
						<tr>
							<td>
								...
								每页显示项数<select type="select" id="currentCount" onChange="changePageNum()">
									...
									<ul>
										<c:if test="${bean.currentPage!=1}">
											<li class="nextPage">
												<a href="${pageContext.request.contextPath}/showProductByPage?currentPage=${bean.currentPage-1}&category=${bean.category}&currentCount=${bean.currentCount}">&lt;&lt;上一页</a>
											</li>
										</c:if>
										<c:if test="${bean.currentPage==1}">
										...
										<c:forEach begin="1" end="${bean.totalPage}" var="pageNum">
											...
											<c:if test="${pageNum!=bean.currentPage}">
												<li><a href="${pageContext.request.contextPath}/showProductByPage?currentPage=${pageNum}&category=${bean.category}&currentCount=${bean.currentCount}">${pageNum}</a>
												</li>
											</c:if>
										</c:forEach>
										...
										<c:if test="${bean.currentPage!=bean.totalPage&&bean.totalPage!=0}">
											<li class="nextpage">
												<a href="${pageContext.request.contextPath}/showProductByPage?currentPage=${bean.currentPage+1}&category=${bean.category}&currentCount=${bean.currentCount}">下一页
													&gt;&gt;</a>
											</li>
										</c:if>
									</ul>
